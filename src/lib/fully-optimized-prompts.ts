...
export const createDynamicResumePrompt = (originalResume: string, jobDescription: string, analysisResult: string): string => {
  return `
    ...
    **For Content That's Too Short:**
    ...
    ## SMART CONTENT DECISIONS BASED ON ORIGINAL RESUME
    - Analyze the original resume pattern and content
    - Include workExperience ONLY if it's present in the original resume
    - Do not fabricate or invent work experience that's not in the original
    - Enhance what exists, don't create what doesn't

    ## 🚀 REQUIRED JSON OUTPUT (DYNAMIC BASED ON ORIGINAL CONTENT)
...
export const createIntelligentLayoutPrompt = (resumeJson: string, analysisResult?: string): string => {
  return `
    ...
    **For Experienced Professionals:**
    ...
    ## SMART WORK EXPERIENCE PRESERVATION

    **CRITICAL RULE**: Never fabricate work experience that doesn't exist in the original resume.
    - If workExperience exists in the input JSON, optimize it while preserving content
    - If workExperience does not exist, do not add it
    - Always maintain the integrity of the original content structure
    ...
    Note: The "workExperience" array should be included ONLY if it exists in the input JSON. Do not add it if it wasn't present originally.
    \`\`\`
  `;
};
